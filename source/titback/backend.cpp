...
TIT_IMPLEMENT_MAIN([](int /*argc*/, char** argv) {
  ...
  CROW_WEBSOCKET_ROUTE(app, "/ws")
      .onmessage([&storage](crow::websocket::connection& connection,
        ...
        const auto& message = request["message"].get<std::string>();
        if (message.empty()) {
          response["result"] = storage.last_series().num_frames();
        } else {
          const auto frame_index = std::stoul(message);
          const auto frame = (storage.last_series().frames() |
                              std::ranges::to<std::vector>())[frame_index];
