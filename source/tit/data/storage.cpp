...
auto DataStorage::create_array_id(DataSetID dataset_id,
  ...
  statement.run(dataset_id.get(), name, type.id(), data_source.pull(npos));
...
void DataStorage::read_array_data(DataArrayID array_id,
  ...
  data_sink.push(statement.column<BlobView>());
}

auto DataStorage::array_data(DataArrayID array_id) const -> Blob {
  Blob result{};
