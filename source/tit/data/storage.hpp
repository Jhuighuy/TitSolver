...
public:
  ...
  auto name() const -> std::string {
    return storage().param_name(param_id_);
  }

  /// Get the type of the parameter.
  auto type() const -> std::string {
    return storage().param_type(param_id_);
  }
...
public:
  ...
  auto series_params(this auto& self, DataSeriesID series_id) {
    return std::views::transform(
        self.series_param_ids(series_id),
  ...
  auto param_name(DataParamID param_id) const -> std::string;

  /// Get the type of a parameter.
  auto param_type(DataParamID param_id) const -> std::string;
