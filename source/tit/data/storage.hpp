...
/// Data series ID type.
using DataSeriesID = Strict<uint64_t, impl::DataSeriesTag>;

/// Data time step ID type.
using DataTimeStepID = Strict<uint64_t, impl::DataTimeStepTag>;

/// Dataset ID type.
using DataSetID = Strict<uint64_t, impl::DataSetTag>;

/// Data array ID type.
using DataArrayID = Strict<uint64_t, impl::DataArrayTag>;
...
public:
  ...
  constexpr operator DataArrayID() const noexcept {
    return array_id_;
  }
  ...
  auto data() const -> std::vector<Val> {
    TIT_ASSERT(storage_ != nullptr, "Data storage was not set!");
    return storage_->template array_data<Val>(array_id_);
  }
...
public:
  ...
  /// Get the dataset ID.
  constexpr operator DataSetID() const noexcept {
    return dataset_id_;
  }
...
public:
  ...
  constexpr operator DataTimeStepID() const noexcept {
...
public:
  ...
  constexpr operator DataSeriesID() const noexcept {
    return series_id_;
  }
...
public:
  ...
  auto find_metadata(std::string_view key) const -> std::optional<std::string>;

  /// Set the metadata value.
  void set_metadata(std::string_view key, std::string_view value);

  /// Enumerate all metadata values.
  auto enumerate_metadata() const
      -> std::vector<std::pair<std::string, std::string>>;

  /// Delete the metadata value.
  void delete_metadata(std::string_view key);
  ...
  /// @{
  void read_array_data(DataArrayID array_id, DataSink& data_sink) const;
  auto array_data(DataArrayID array_id) const -> Blob;
  template<data_type Val>
  auto array_data(DataArrayID array_id) const -> std::vector<Val> {
    std::vector<Val> result{};
    auto data_sink = make_iter_data_sink<Val>(std::back_inserter(result));
...
private:
  ...
  static constexpr size_t max_series_ = 5;
  mutable sqlite::Database db_;
  mutable zstd::Compressor compressor_;
  mutable zstd::Decompressor decompressor_;
