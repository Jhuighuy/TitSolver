...
TEST_CASE("data::DataStorage") {
  ...
  SUBCASE("failure") {
    ...
    SUBCASE("invalid file") {
      // Current source file definitely is not a valid `.ttdb` file.
...
TEST_CASE("data::DataArrayView") {
  SUBCASE("empty dataset") {
    ...
    // CHECK(std::ranges::empty(frame.arrays()));
  }
  SUBCASE("create arrays") {
    ...
    const auto array_1 = frame.create_array( //
        "array_1",
    ...
    const auto array_2 = frame.create_array( //
        "array_2",
  ...
  SUBCASE("find arrays") {
    ...
    const auto array_1 =
        frame.create_array("array_1", std::vector{std::numbers::pi});
    REQUIRE(storage.check_array(array_1));

    const auto array_2 =
        frame.create_array("array_2", std::vector{std::numbers::e});
    REQUIRE(storage.check_array(array_2));
  ...
  SUBCASE("update arrays") {
    ...
    const auto array =
        frame.create_array("array", std::vector{std::numbers::pi});
  ...
  SUBCASE("delete arrays") {
    ...
    const auto array_3 =
        frame.create_array("array_3", std::vector{std::numbers::phi});
