...
public:
  ...
  void write_frame(const std::string& frame_name,
    ...
    for (const auto& array : frame.arrays()) {
      ...
      std::vector<std::byte> array_data(array_size * array_type.width());
      array.read(std::span{array_data});
...
public:
  ...
  void write_frame(const std::filesystem::path& hdf5_rel_path,
    ...
    TIT_ALWAYS_ASSERT(positions.has_value(), "Positions array 'r' not found!");
