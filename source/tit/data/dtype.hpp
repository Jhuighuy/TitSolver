...
public:
  ...
  consteval Dtype(Dkind kind, uint16_t size_of, uint8_t dim, uint8_t rank)
      : kind_{kind}, size_of_{size_of}, dim_{dim}, rank_{rank} {
  ...
  constexpr auto size_of() const noexcept -> size_t {
    return size_of_;
  }
...
template<class Val>
struct dtype_of :
    value_constant<Dtype{dkind_of_v<Val>,
                         sizeof(Val),
                         /*dim=*/1,
                         /*rank=*/0}> {};
...
struct dtype_of<Vec<Num, Dim>> :
    value_constant<Dtype{dkind_of_v<Num>,
                         sizeof(Vec<Num, Dim>),
                         Dim,
                         /*rank=*/1}> {};
