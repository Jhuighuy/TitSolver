...
TEST_CASE("data::Kind") {
  SUBCASE("correct") {
    const data::Kind kind{data::Kind::ID::float32};
    CHECK(kind.id() == data::Kind::ID::float32);
    CHECK(kind.name() == "float32_t");
    CHECK(kind.width() == 4);
  }
  SUBCASE("incorrect") {
    // NOLINTNEXTLINE(clang-analyzer-optin.core.EnumCastOutOfRange)
    CHECK_THROWS_MSG(data::Kind{data::Kind::ID{137}},
                     Exception,
...
TEST_CASE("data::Type") {
  SUBCASE("correct") {
    SUBCASE("scalar") {
      const data::Type type{data::kind_of<float32_t>};
      CHECK(type.kind() == data::kind_of<float32_t>);
      CHECK(type.rank() == data::Rank::scalar);
      CHECK(type.dim() == 1);
      CHECK(type.width() == 4);
      CHECK(type.name() == "float32_t");
    }
    SUBCASE("vector") {
      const data::Type type{data::kind_of<float64_t>, data::Rank::vector, 2};
  ...
  SUBCASE("incorrect") {
    ...
    SUBCASE("invalid dim") {
      CHECK_THROWS_MSG(
          data::Type(data::kind_of<float32_t>, data::Rank::vector, 0),
          Exception,
          "Dimensionality must be positive, but is 0.");
    }
    SUBCASE("invalid scalar dim") {
      CHECK_THROWS_MSG(
          data::Type(data::kind_of<float32_t>, data::Rank::scalar, 2),
          Exception,
  ...
  SUBCASE("ID") {
    ...
    SUBCASE("from ID") {
      SUBCASE("valid") {
        CHECK(data::Type{0x030209} == data::type_of<Mat<float32_t, 3>>);
      }
      SUBCASE("invalid") {
        CHECK_THROWS_MSG(data::Type{0x1337}, Exception, "Invalid");
      }
    }
  }
}
