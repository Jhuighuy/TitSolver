...
public:
  ...
  constexpr auto operator()(Points&& points,
    ...
    using PermSubrange = std::ranges::borrowed_subrange_t<Perm>;
    PermSubrange right_perm;
    if (reverse) {
    ...
    PermSubrange left_perm{std::begin(perm), std::begin(right_perm)};
    return {std::move(left_perm), std::move(right_perm)};
  }
...
public:
  ...
  constexpr auto operator()(Points&& points,
    ...
    PermSubrange left_perm{std::begin(perm), std::begin(right_perm)};
    return std::pair{std::move(left_perm), std::move(right_perm)};
  }
