...
// A sketch of a 8x8 Morton curve.
//
//       0    1    2    3      4    5    6    7
//   0 --|----|----|----|------|----|----|----|--->
//   |                                             Y
...
//    X
//
constexpr auto points_on_8x8_morton_curve = std::to_array<Vec2D>({
...
TEST_CASE("geom::MortonCurveSort") {
  ...
  // Ensure the points are sorted correctly.
  const auto perm = sfc.perm();
  for (size_t i = 0; i < points.size(); ++i) {
    CHECK(all(points[perm[i]] == points_on_8x8_morton_curve[i]));
  }
  const auto iperm = sfc.iperm();
  for (size_t i = 0; i < points.size(); ++i) {
    CHECK(all(points[i] == points_on_8x8_morton_curve[iperm[i]]));
  }
}
