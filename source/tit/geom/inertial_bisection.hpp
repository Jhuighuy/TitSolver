...
public:
  ...
  InertialBisection(Points points, size_t num_parts)
    ...
    const auto num_points = std::ranges::size(points_);
...
private:
  ...
  void partition_(par::TaskGroup& tasks,
                  std::span<size_t> range,
                  size_t num_parts,
                  size_t part_index) {
    TIT_ASSERT(std::has_single_bit(num_parts),
               "Number of parts must be a power of two at the moment.");
    if (num_parts == 1) {
      // No further partitioning, assign the part index to points.
      for (const auto index : range) parts_[index] = part_index;
      return;
    }
...
InertialBisection(Points&&,
                  Args...) -> InertialBisection<std::views::all_t<Points>>;
