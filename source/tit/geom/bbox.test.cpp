...
TEST_CASE("geom::BBox::center_and_extents") {
  const geom::BBox bbox{Vec{1.0, 1.0}, Vec{3.0, 3.0}};
...
TEST_CASE("geom::BBox::clamp") {
  const geom::BBox bbox{Vec{0.0, 0.0}, Vec{2.0, 2.0}};
...
TEST_CASE("geom::BBox::split") {
  const geom::BBox bbox{Vec{0.0, 0.0}, Vec{2.0, 2.0}};
  const auto [left, right] = bbox.split(0, 0.5);
