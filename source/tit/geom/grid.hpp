...
public:
  ...
  /// Initialize a grid with the given bounding box and one cell.
  constexpr explicit Grid(const Box& box) : box_{box} {
    set_num_cells(VecIndex(1));
  }
  ...
  constexpr auto extend(size_t amount) -> Grid& {
    ...
    box_.grow(cell_extents_ * amount);
    return *this;
  }
  ...
  constexpr auto cells() const noexcept {
    return cells(VecIndex(0), num_cells_);
  }
  ...
  /// Range of internal cell indices, such that `n <= index < num_cells - n`.
  constexpr auto internal_cells(size_t n) const noexcept {
    return cells(VecIndex(n), num_cells_ - VecIndex(n));
  }

  /// Range of cell indices that intersect the given box.
  constexpr auto cells_intersecting(const Box& box) const noexcept {
    const auto half_cell_extents = cell_extents_ / 2;
    const auto safe_intersection = auto{box}
