...
constexpr auto small_number_v<Strict<Real>> = Strict{small_number_v<Real>};
...
/// Number addition.
template<class Num>
constexpr auto operator+(Strict<Num> const& a, Strict<Num> const& b)
    -> Strict<Num> {
...
template<class Num>
constexpr auto operator+=(Strict<Num>& a, Strict<Num> const& b)
    -> Strict<Num>& {
  a.get() += b.get();
  return a;
}
...
template<class Num>
constexpr auto pow(Strict<Num> const& a, std::type_identity_t<Num> const& b)
    -> Strict<Num> {
