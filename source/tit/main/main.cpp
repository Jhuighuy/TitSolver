...
void println_logo_and_system_info() {
  ...
  /// @todo Check and compare this implementation against what we have in a
  ///       different place.
  try {
  ...
  try {
    info_lines.push_back(
        std::format("Work Dir ....... {}", std::filesystem::current_path()));
  } catch (const Exception& e) {
    err("Unable to get working directory: {}.", e.what());
  }

  try {
    info_lines.push_back(std::format("Disk space ..... {}",
                                     fmt_memsize(sys_info::disk_space())));
  } catch (const Exception& e) {
