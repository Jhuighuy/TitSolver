...
q, s, z, eta, rho, delta, beta, A, L = sp.symbols(
    "q s z eta rho delta beta A L",
)
...
def normalized_line_expr(expr: sp.Expr) -> sp.Expr:
    return sp.collect(sp.expand(expr), [A, L, rho, beta])
...
def line_primitive_function(name: str, expr: sp.Expr) -> list[str]:
    ...
    used_symbols = {
        symbol
        for symbol in (eta, delta, z, rho, A, L)
