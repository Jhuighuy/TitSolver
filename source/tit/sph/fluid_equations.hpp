...
public:
  ...
  constexpr explicit FluidEquations(Num g,
      ...
      : g_{g},                                                 //
        domain_{std::move(domain)}, containment_{containment}, //
        eos_{std::move(eos)},                                  //
  ...
  auto compute_time_step(ParticleArray& particles) const -> Num {
    ...
    return par::fold(
        ...
        [this](Num dt, PV a) {
          ...
          // Diffusion time step.
          const auto dt_diff =
              C_diff_ * pow2(h[a]) * rho[a] / std::max(mu[a], kappa[a]);
          ...
          return std::min({dt, dt_acoustic, dt_diff, dt_force});
        },
