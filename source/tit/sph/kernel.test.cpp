...
#define KERNEL_TYPES                                                           \
  TIT_PASS(sph::GaussianKernel<float64_t>,                                     \
           sph::CubicSplineKernel<float64_t>,                                  \
           sph::QuarticSplineKernel<float64_t>,                                \
           sph::QuinticSplineKernel<float64_t>,                                \
           sph::QuarticWendlandKernel<float64_t>,                              \
           sph::SixthOrderWendlandKernel<float64_t>,                           \
           sph::EighthOrderWendlandKernel<float64_t>)
  ...
  SUBCASE("over box") {
    SUBCASE("1D") {
      for (const double h : {1.0, 0.1, 0.01}) {
        ...
        const Kernel w{h};
        const auto r = w.radius(h);
        const auto i = integrate(w, Box1D{-r, +r});
        CHECK(approx_equal_to(i, 1.0));
      }
    }
    SUBCASE("2D") {
      for (const double h : {1.0, 0.1, 0.01}) {
        ...
        const Kernel w{h};
        const auto r = w.radius(h);
        const auto i = integrate(w, Box2D{{-r, -r}, {+r, +r}});
        CHECK(approx_equal_to(i, 1.0));
      }
    }
    SUBCASE("3D") {
      for (const double h : {1.0, 0.1, 0.01}) {
        ...
        const Kernel w{h};
        const auto r = w.radius(h);
        const auto i = integrate(w, Box3D{{-r, -r, -r}, {+r, +r, +r}});
        CHECK(approx_equal_to(i, 1.0));
      }
    }
  }
...
TEST_CASE_TEMPLATE("sph::Kernel::grad", Kernel, KERNEL_TYPES) {
  ...
  SUBCASE("single component") {
    for (const double h : {1.0, 0.1, 0.01}) {
      const Kernel<Dual<double>> w{h};
      const auto x = pow2(h) * Vec{0.1, 0.1, 0.1};
      const auto value = w(Vec{Dual{x[0], 1.0}, Dual{x[1]}, Dual{x[2]}});
