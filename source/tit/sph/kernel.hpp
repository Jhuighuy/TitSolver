...
public:
  ...
  constexpr auto radius(this const auto& self, PV a) noexcept
      -> particle_num_t<PV> {
  ...
  template<particle_view PV>
  constexpr auto width_deriv(this const auto& self, PV a, PV b) noexcept
      -> particle_num_t<PV> {
  ...
  /// Visit the kernel implementation.
  template<class Self, std::invocable<const Self&> Visitor>
  constexpr auto _visit(this const Self& self, Visitor visitor) noexcept {
    return std::invoke(visitor, self);
  }
  ...
  constexpr auto radius(this const auto& self, Num h) noexcept -> Num {
    ...
    return self._visit([h](const auto& impl) { //
      return impl.template _unit_radius<Num>() * h;
    });
  }
  ...
  constexpr auto operator()(this const auto& self,
    ...
    TIT_ASSERT(h > Num{0.0}, "Kernel width must be positive!");
    return self._visit([&x, h](const auto& impl) {
      const auto h_inverse = inverse(h);
      const auto w = impl.template weight<Num, Dim>() * pow(h_inverse, Dim);
      const auto q = h_inverse * norm(x);
      return w * impl._unit_value(q);
    });
  }
  ...
  constexpr auto grad(this const auto& self,
    ...
    return self._visit([&x, h](const auto& impl) {
      ...
      return w * impl._unit_deriv(q) * grad_q;
    });
  }
  ...
  constexpr auto width_deriv(this const auto& self,
    ...
    return self._visit([&x, h](const auto& impl) {
      ...
      return dw_dh * impl._unit_value(q) + w * impl._unit_deriv(q) * dq_dh;
    });
  }
...
public:
  ...
  /// Unit support radius.
  template<class Num>
  static constexpr auto _unit_radius() noexcept -> Num {
    return Num{2.0};
  }

  /// Value of the unit smoothing kernel at a point.
  template<class Num>
  static constexpr auto _unit_value(Num q) noexcept -> Num {
  ...
  template<class Num>
  static constexpr auto _unit_deriv(Num q) noexcept -> Num {
    constexpr Vec qi{Num{2.0}, Num{1.0}};
...
public:
  ...
  /// Value of the unit smoothing kernel at a point.
  template<class Num>
  constexpr auto _unit_value(this auto& self, Num q) noexcept -> Num {
    return q < Num{2.0} ? self.unit_value_notrunc(q) : Num{0.0};
  }

  /// Derivative of the unit smoothing kernel at a point.
  template<class Num>
  constexpr auto _unit_deriv(this auto& self, Num q) noexcept -> Num {
...
class KernelVariant final : public Kernel {
public:

  /// Construct a kernel variant.
  template<class Impl>
  constexpr explicit KernelVariant(Impl impl) noexcept
      : impl_{std::move(impl)} {}

  /// Visit the kernel implementation.
  constexpr auto _visit(auto visitor) const noexcept {
    return std::visit(std::move(visitor), impl_);
  }
...
/// Dynamically dispatched kernel.
using DKernel = KernelVariant<CubicSplineKernel,
                              QuarticSplineKernel,
                              QuinticSplineKernel,
...
concept kernel = std::same_as<K, CubicSplineKernel> ||
                 ...
                 specialization_of<K, KernelVariant>;
