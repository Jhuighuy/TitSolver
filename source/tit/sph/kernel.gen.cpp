...
const Expr Dim{se::symbol("Dim")};     ///< Spatial dimension.
...
/// @todo Differs from `j_expr` only by replacing `eta` with `beta`.
...
public:
  ...
  auto antigrad_flux_primitive() const -> OptExpr {
    const Poly poly{tail_moment().subs(Dim, 2), q};
    Expr result{0};
    for (const auto& [p, c] : poly.terms()) {
      if (p == 0) result += c * A;
  ...
  auto antigrad_flux_line_primitive() const -> OptExpr {
    const Poly poly{tail_moment().subs(Dim, 3), q};
    Expr result{0};
    for (const auto& [p, c] : poly.terms()) {
      if (p == 0) {
...
void emit_antideriv_moment(std::ostream& os, const Kernel& kernel) {
  const auto make_terms = [&kernel](int dim) {
    ...
    for (const auto& segment : kernel.segments()) {
      terms.emplace_back(
          truncated(segment, segment.tail_moment().subs(Dim, dim)));
    }
    return terms;
  };
  std::println(os, "template<>");
