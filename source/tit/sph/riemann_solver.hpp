...
public:
  ...
  constexpr auto operator()(PV a, PV b) const noexcept {
    ...
    const auto alpha = dot(v[b, a], e_ab);
    const auto beta = std::clamp(3 * alpha, zero(alpha), cs_0_);
    const auto p_ast = p.avg(a, b) + beta * rho_ab * alpha / 2;
    const auto v_ast = v.avg(a, b) + p[b, a] / (2 * rho_ab * cs_0_) * e_ab;
