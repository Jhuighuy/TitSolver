...
public:
  ...
  virtual auto pull(size_t count) -> BlobView = 0;
...
public:
  ...
  /// Pull the at most `count` bytes from the data source.
  constexpr auto pull(size_t count) -> BlobView override {
...
public:
  ...
  virtual void push(BlobView bytes) = 0;
...
public:
  ...
  /// Push the bytes to the data sink.
  constexpr void push(BlobView bytes) override {
...
template<serializable Val = std::byte, std::output_iterator<Val> Iter>
