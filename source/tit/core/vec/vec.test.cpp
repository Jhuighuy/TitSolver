...
TEST_CASE_TEMPLATE("Vec", Num, NUM_TYPES) {
  ...
  SUBCASE("value initialization") {
    Vec<Num, 2> const v(Num{3});
    CHECK(v[0] == Num{3});
    CHECK(v[1] == Num{3});
  }
  SUBCASE("aggregate initialization") {
    Vec const v{Num{1}, Num{2}};
    CHECK(v[0] == Num{1});
    CHECK(v[1] == Num{2});
  }
...
TEST_CASE_TEMPLATE("Vec::operator+", Num, NUM_TYPES) {
  SUBCASE("normal") {
    CHECK(all(Vec{Num{1}, Num{2}} + Vec{Num{3}, Num{4}} == //
              Vec{Num{4}, Num{6}}));
  }
...
TEST_CASE_TEMPLATE("Vec::operator-", Num, NUM_TYPES) {
  ...
  SUBCASE("subtraction") {
    SUBCASE("normal") {
      CHECK(all(Vec{Num{3}, Num{4}} - Vec{Num{1}, Num{2}} ==
                Vec{Num{2}, Num{2}}));
    }
    SUBCASE("with assignment") {
      Vec v{Num{3}, Num{4}};
...
TEST_CASE_TEMPLATE("Vec::operator*", Num, NUM_TYPES) {
  ...
  SUBCASE("multiplication") {
    SUBCASE("normal") {
      CHECK(all(Vec{Num{2}, Num{3}} * Vec{Num{4}, Num{5}} ==
                Vec{Num{8}, Num{15}}));
    }
    SUBCASE("with assignment") {
      Vec v{Num{2}, Num{3}};
...
TEST_CASE_TEMPLATE("Vec::operator/", Num, NUM_TYPES) {
  ...
  SUBCASE("division") {
    SUBCASE("normal") {
      CHECK(all(Vec{Num{8}, Num{15}} / Vec{Num{2}, Num{3}} ==
                Vec{Num{4}, Num{5}}));
    }
    SUBCASE("with assignment") {
      Vec v{Num{8}, Num{15}};
...
TEST_CASE_TEMPLATE("Vec::operator==", Num, NUM_TYPES) {
  CHECK(all(Vec{Num{1}, Num{2}} == Vec{Num{1}, Num{2}}));
  CHECK_FALSE(any(Vec{Num{1}, Num{2}} == Vec{Num{3}, Num{4}}));
}

TEST_CASE_TEMPLATE("Vec::operator!=", Num, NUM_TYPES) {
  CHECK(all(Vec{Num{1}, Num{2}} != Vec{Num{3}, Num{4}}));
  CHECK_FALSE(any(Vec{Num{1}, Num{2}} != Vec{Num{1}, Num{2}}));
}

TEST_CASE_TEMPLATE("Vec::operator<", Num, NUM_TYPES) {
  CHECK(all(Vec{Num{1}, Num{2}} < Vec{Num{3}, Num{4}}));
  CHECK_FALSE(any(Vec{Num{3}, Num{4}} < Vec{Num{1}, Num{2}}));
}
...
TEST_CASE_TEMPLATE("Vec::cross", Num, NUM_TYPES) {
  CHECK(all(                             //
      cross(Vec{Num{1}, Num{0}, Num{0}}, //
            Vec{Num{0}, Num{1}, Num{0}}) == Vec{Num{0}, Num{0}, Num{1}}));
  CHECK(all(                             //
      cross(Vec{Num{1}, Num{2}, Num{3}}, //
