...
#include "tit/core/mat/utils.hpp"
...
/// LU matrix factorization: `A = L * U`,
/// where `L` is a lower-triangular matrix with unit diagonal and `U` is an
...
public:
  ...
  constexpr auto solve(Arg x) const -> Arg {
    solve_part<MatPart::lower_unit>(LU_, x);
    solve_part<MatPart::upper_diag>(LU_, x);
    return x;
  }
...
private:
  ...
  template<class Num_, size_t Dim_>
  friend constexpr auto lu(Mat<Num_, Dim_> const& A)
      -> FactResult<FactLU<Num_, Dim_>>;
...
/// Cholesky matrix factorization: `A = L * L^T`,
...
public:
  ...
  template<class Arg>
    requires (std::same_as<Arg, Vec<Num, Dim>> ||
              std::same_as<Arg, Mat<Num, Dim>>)
  constexpr auto solve(Arg x) const -> Arg {
    solve_part<MatPart::lower_diag>(L_, x);
    solve_part_transposed<MatPart::lower_diag>(L_, x);
    return x;
  }
...
private:
  ...
  template<class Num_, size_t Dim_>
  friend constexpr auto chol(Mat<Num_, Dim_> const& A)
      -> FactResult<FactChol<Num_, Dim_>>;
...
public:
  ...
  template<class Arg>
    requires (std::same_as<Arg, Vec<Num, Dim>> ||
              std::same_as<Arg, Mat<Num, Dim>>)
  constexpr auto solve(Arg x) const -> Arg {
    solve_part<MatPart::lower_unit>(LD_, x);
    solve_part<MatPart::diag>(LD_, x);
    solve_part_transposed<MatPart::lower_unit>(LD_, x);
    return x;
  }
...
private:
  ...
  template<class Num_, size_t Dim_>
  friend constexpr auto ldl(Mat<Num_, Dim_> const& A)
      -> FactResult<FactLDL<Num_, Dim_>>;
