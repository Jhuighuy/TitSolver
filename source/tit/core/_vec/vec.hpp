...
public:
  ...
  /// @{
  constexpr auto operator[](std::size_t i) noexcept -> Num& {
    TIT_ASSERT(i < Dim, "Row index is out of range!");
    return col_[i];
  }
  constexpr auto operator[](std::size_t i) const noexcept -> const Num& {
    TIT_ASSERT(i < Dim, "Row index is out of range!");
    return col_[i];
  }
  /// @}
  ...
  /// @{
  auto reg(std::size_t i) noexcept -> Reg& {
    TIT_ASSERT(i < RegCount, "Register index is out of range!");
    return regs_[i];
  }
  auto reg(std::size_t i) const noexcept -> const Reg& {
    TIT_ASSERT(i < RegCount, "Register index is out of range!");
    return regs_[i];
  }
  /// @}
  ...
  friend constexpr auto operator+(const Vec& a, const Vec& b) -> Vec {
    Vec r;
    TIT_IF_SIMD_AVALIABLE(Num) {
      for (std::size_t i = 0; i < RegCount; ++i) r.reg(i) = a.reg(i) + b.reg(i);
      return r;
    }
  ...
  friend constexpr auto operator+=(Vec& a, const Vec& b) -> Vec& {
    TIT_IF_SIMD_AVALIABLE(Num) {
      for (std::size_t i = 0; i < RegCount; ++i) a.reg(i) += b.reg(i);
      return a;
    }
  ...
  friend constexpr auto operator-(const Vec& a) -> Vec {
    Vec r;
    TIT_IF_SIMD_AVALIABLE(Num) {
      for (std::size_t i = 0; i < RegCount; ++i) r.reg(i) = -a.reg(i);
      return r;
    }
  ...
  friend constexpr auto operator*(const Vec& a, const Num& b) -> Vec {
    ...
    TIT_IF_SIMD_AVALIABLE(Num) {
      const Reg b_reg(b);
      for (std::size_t i = 0; i < RegCount; ++i) r.reg(i) = a.reg(i) * b_reg;
      return r;
    }
  ...
  friend constexpr auto operator*=(Vec& a, const Num& b) -> Vec& {
    TIT_IF_SIMD_AVALIABLE(Num) {
      const Reg b_reg(b);
      for (std::size_t i = 0; i < RegCount; ++i) a.reg(i) *= b_reg;
      return a;
    }
  ...
  friend constexpr auto operator==(const Vec& a, const Vec& b) -> VecMask {
    ...
    for (std::size_t i = 0; i < Dim; ++i) m[i] = a[i] == b[i];
    return m;
  }
  ...
  friend constexpr auto operator<(const Vec& a, const Vec& b) -> VecMask {
    VecMask m;
    TIT_IF_SIMD_AVALIABLE(Num) {
      for (std::size_t i = 0; i < RegCount; ++i) m.reg(i) = a.reg(i) < b.reg(i);
      return m;
    }
    for (std::size_t i = 0; i < Dim; ++i) m[i] = a[i] < b[i];
    return m;
...
/// Make a unit vector.
template<std::size_t Axis = 0, class Num, std::size_t Dim>
...
template<class Num, std::size_t Dim1, std::size_t Dim2>
constexpr auto vec_cat(const Vec<Num, Dim1>& a, const Vec<Num, Dim2>& b)
    -> Vec<Num, Dim1 + Dim2> {
  Vec<Num, Dim1 + Dim2> r;
  for (std::size_t i = 0; i < Dim1; ++i) r[i] = a[i];
  for (std::size_t i = 0; i < Dim2; ++i) r[i + Dim1] = b[i];
  return r;
}

/// Extract the head part of the vector.
template<std::size_t HeadDim = 1, class Num, std::size_t Dim>
constexpr auto vec_head(const Vec<Num, Dim>& a) -> Vec<Num, HeadDim> {
  static_assert(HeadDim <= Dim, "Head dimension is out of range!");
  Vec<Num, HeadDim> r;
  for (std::size_t i = 0; i < HeadDim; ++i) r[i] = a[i];
  return r;
}
...
constexpr auto vec_tail(const Vec<Num, Dim>& a) -> Vec<Num, Dim - HeadDim> {
  ...
  for (std::size_t i = 0; i < Dim - HeadDim; ++i) r[i] = a[i + HeadDim];
  return r;
}

/// Element-wise vector cast.
template<class To, class From, std::size_t Dim>
constexpr auto vec_cast(const Vec<From, Dim>& a) -> Vec<To, Dim> {
  Vec<To, Dim> r;
  TIT_IF_SIMD_CAST_AVALIABLE(From, To) {
    for (std::size_t i = 0; i < Vec<From, Dim>::RegCount; ++i) {
      r.reg(i) = simd::reg_cast<To>(a.reg(i));
    }
    return r;
  }
  for (std::size_t i = 0; i < Dim; ++i) r[i] = static_cast<To>(a[i]);
  return r;
}
...
constexpr auto minimum(const Vec<Num, Dim>& a, const Vec<Num, Dim>& b)
  ...
  for (std::size_t i = 0; i < Dim; ++i) r[i] = std::min(a[i], b[i]);
  return r;
}
...
constexpr auto maximum(const Vec<Num, Dim>& a, const Vec<Num, Dim>& b)
  ...
  for (std::size_t i = 0; i < Dim; ++i) r[i] = std::max(a[i], b[i]);
  return r;
}
...
constexpr auto filter(const VecMask<Num, Dim>& m, const Vec<Num, Dim>& a)
  ...
  for (std::size_t i = 0; i < Dim; ++i) r[i] = m[i] ? a[i] : Num{0};
  return r;
}
...
constexpr auto select(const VecMask<Num, Dim>& m,
  ...
  for (std::size_t i = 0; i < Dim; ++i) r[i] = m[i] ? a[i] : b[i];
  return r;
}
...
constexpr auto floor(const Vec<Num, Dim>& a) -> Vec<Num, Dim> {
  ...
  for (std::size_t i = 0; i < Dim; ++i) r[i] = floor(a[i]);
  return r;
}
...
constexpr auto round(const Vec<Num, Dim>& a) -> Vec<Num, Dim> {
  ...
  for (std::size_t i = 0; i < Dim; ++i) r[i] = round(a[i]);
  return r;
}
...
constexpr auto ceil(const Vec<Num, Dim>& a) -> Vec<Num, Dim> {
  ...
  for (std::size_t i = 0; i < Dim; ++i) r[i] = ceil(a[i]);
  return r;
}
...
constexpr auto sum(const Vec<Num, Dim>& a) -> Num {
  ...
  auto r = a[0];
  for (std::size_t i = 1; i < Dim; ++i) r += a[i];
  return r;
}
...
constexpr auto min_value(const Vec<Num, Dim>& a) -> Num {
  ...
  auto r = a[0];
  for (std::size_t i = 1; i < Dim; ++i) r = std::min(r, a[i]);
  return r;
}
...
constexpr auto max_value(const Vec<Num, Dim>& a) -> Num {
  ...
  auto r = a[0];
  for (std::size_t i = 1; i < Dim; ++i) r = std::max(r, a[i]);
  return r;
}

/// Index of the minimal vector element.
template<class Num, std::size_t Dim>
constexpr auto min_value_index(const Vec<Num, Dim>& a) -> std::size_t {
  std::size_t ir = 0;
...
template<class Num, std::size_t Dim>
constexpr auto max_value_index(const Vec<Num, Dim>& a) -> std::size_t {
  std::size_t ir = 0;
  for (std::size_t i = 1; i < Dim; ++i) {
    if (a[i] > a[ir]) ir = i;
  }
...
constexpr auto dot(const Vec<Num, Dim>& a, const Vec<Num, Dim>& b) -> Num {
  ...
  auto r = a[0] * b[0];
  for (std::size_t i = 1; i < Dim; ++i) r += a[i] * b[i];
  return r;
}
