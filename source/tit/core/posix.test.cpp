...
TEST_CASE("checked_open/checked_close") {
  ...
  SUBCASE("failure") {
    ...
    SUBCASE("invalid flags") {
      CHECK_THROWS_MSG(checked_open(__FILE__, -1),
                       ErrnoException,
                       "Failed to open a file.");
    }
...
TEST_CASE("checked_read") {
  SUBCASE("success") {
    ...
    CHECK(std::string_view{buffer.data()} == message);
  ...
  SUBCASE("failure") {
    ...
    SUBCASE("closed descriptor") {
      const auto fd = checked_open(__FILE__, O_RDONLY);
      checked_close(fd);

      std::array<char, 20> buffer{};
      CHECK_THROWS_MSG(checked_read(fd, buffer),
                       ErrnoException,
                       "Failed to read from a file descriptor.");
    }
  }
...
TEST_CASE("checked_write") {
  SUBCASE("success") {
    ...
    const auto fd = checked_open(file_name, O_WRONLY | O_CREAT | O_TRUNC);
  ...
  SUBCASE("failure") {
    ...
    SUBCASE("closed descriptor") {
      ...
      CHECK_THROWS_MSG(checked_write(fd, "Hello, World!"),
                       ErrnoException,
                       "Failed to write to a file descriptor.");
    }
  }
...
TEST_CASE("checked_dup2") {
  ...
  SUBCASE("failure") {
    SUBCASE("invalid file descriptor") {
      CHECK_THROWS_MSG(checked_dup2(99, 66), ErrnoException, "`dup2` failed.");
    }
  }
}
...
TEST_CASE("checked_get_fcntl") {
  SUBCASE("success") {
    const auto fd = checked_open("test.txt", O_WRONLY | O_CREAT);
  ...
  SUBCASE("failure") {
    ...
    SUBCASE("closed descriptor") {
      ...
      CHECK_THROWS_MSG(checked_get_fcntl(fd),
                       ErrnoException,
                       "Failed to get a file descriptor flags.");
    }
  }
...
TEST_CASE("checked_set_fcntl") {
  ...
  SUBCASE("failure") {
    ...
    SUBCASE("closed descriptor") {
      ...
      CHECK_THROWS_MSG(checked_set_fcntl(fd, O_RDONLY | O_NONBLOCK),
                       ErrnoException,
                       "Failed to set a file descriptor flags.");
    }
  }
