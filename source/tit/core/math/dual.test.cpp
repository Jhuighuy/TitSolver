...
TEST_CASE("Dual::pow") {
  ...
  SUBCASE("dual ** dual") {
    ...
    CHECK(d.val() == e * e);
    CHECK(approx_equal_to(d.deriv(), e * (e + 2.0)));
  }
}
...
TEST_CASE("Dual::log") {
  ...
  CHECK(d.val() == 1.0);
  CHECK(approx_equal_to(d.deriv(), 2.0 / e));
}
...
TEST_CASE("Dual::sin") {
  ...
  CHECK(approx_equal_to(d.val(), 1.0));
  CHECK(approx_equal_to(d.deriv(), 0.0));
}

TEST_CASE("Dual::cos") {
  using std::numbers::pi;
  const auto d = cos(Dual{pi / 2.0, 1.0});
  CHECK(approx_equal_to(d.val(), 0.0));
  CHECK(approx_equal_to(d.deriv(), -1.0));
}
