...
auto fmt_measurement(long double value, std::string_view unit, size_t precision)
    -> std::string {
...
// GCC does not recognize time format attribute from function declarations.
// See: https://gcc.gnu.org/bugzilla/show_bug.cgi?id=39438
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wformat-nonliteral"

auto str_to_tm(const char* fmt, CStrView tm_str) -> std::tm {
  TIT_ASSERT(fmt != nullptr, "Invalid format string pointer!");
  std::tm result{}; // NOLINTNEXTLINE(*-include-cleaner)
  const auto* const end = strptime(tm_str.c_str(), fmt, &result);
  TIT_ENSURE(end != nullptr && *end == '\0',
             "Unable to parse time string '{}' with format '{}'.",
             tm_str,
             fmt);
  return result;
}
...
auto fmt_tm(const char* fmt, const std::tm& tm) -> std::string {
  ...
  const auto status = std::strftime(buffer.data(), buffer.size() - 1, fmt, &tm);
  TIT_ENSURE(status != 0, "Unable to format time with format '{}'.", fmt);
  return buffer.data();
}
