...
#include "tit/core/par/range.hpp"       // IWYU pragma: export
...
struct DeterministicForEachRange final : UnviewInvoker<0, 1> {
  using UnviewInvoker<0, 1>::operator();
  ...
  void operator()(Range&& range, Func func) const {
    ...
    tbb::parallel_for(
        size_t{0},
        count,
        [quotient, remainder, iter, func = std::move(func)](
            size_t thread_index) {
          const auto first = thread_index * quotient + //
                             std::min(thread_index, remainder);
          const auto last = (thread_index + 1) * quotient +
                            std::min(thread_index + 1, remainder);
          const std::ranges::subrange subrange{iter + first, iter + last};
          std::invoke(func, subrange, thread_index);
        },
...
struct DeterministicForEach final {
  template<parallelizable_range Range,
    ...
    deterministic_for_each_range(
        ...
        [func = std::move(func)](std::ranges::view auto subrange,
          ...
          std::ranges::for_each(std::move(subrange),
                                std::bind_back(std::ref(func), thread_index));
        });
  }
};
...
struct FoldRange final : AutoPartitionerInvoker, UnviewInvoker<1, 3> {
  using AutoPartitionerInvoker::operator();
  using UnviewInvoker<1, 3>::operator();
  template<partitioner Partitioner,
           blockable_range Range,
...
struct UnstableCopyIf final : AutoPartitionerInvoker {
  ...
  auto operator()(Partitioner partitioner,
    ...
    for_each_range(
        ...
        std::forward<Range>(range),
        [&index, out, pred = std::move(pred), proj = std::move(proj)](
...
struct Transform final : AutoPartitionerInvoker {
  ...
  auto operator()(Partitioner partitioner,
    ...
    for_each(std::move(partitioner),
             ...
             [func = std::move(func), proj = std::move(proj)](auto arg) {
               std::get<0>(arg) =
...
struct Sort final {
  ...
  void operator()(Range&& range, Compare compare = {}, Proj proj = {}) const {
    ...
    tbb::parallel_sort(
        std::begin(range),
        std::end(range),
        [compare = std::move(compare), proj = std::move(proj)](Ref& a, Ref& b) {
