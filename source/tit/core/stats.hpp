...
public:
  ...
  /// Get the mean value as a string.
  constexpr auto render_avg() const -> std::string override {
    auto result = std::format("{}", sum_.front() / count_);
    for (const auto& val : sum_ | std::views::drop(1)) {
      result += std::format(" {}", val / count_);
    }
    return result;
  }
  ...
  /// Get the minimum value as a string.
  constexpr auto render_min() const -> std::string override {
    auto result = std::format("{}", min_.front());
    for (const auto& val : min_ | std::views::drop(1)) {
  ...
  /// Get the maximum value as a string.
  constexpr auto render_max() const -> std::string override {
    auto result = std::format("{}", max_.front());
    for (const auto& val : max_ | std::views::drop(1)) {
      result += std::format(" {}", val);
    }
    return result;
  }
