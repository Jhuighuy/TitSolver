  ...
  void operator()(const Graph& graph, Parts&& parts, size_t num_parts) const {
    ...
    [&coarsen = coarsen_,
     ...
     num_parts](this const auto& self,
      ...
      // Should we do a recursive coarsening-refinement iteration?
      //
      ...
      if (fine_graph.num_nodes() >= 15 * num_parts) {
        // Coarsen the graph.
        ...
        if (coarse_graph.num_nodes() * 10 <= fine_graph.num_nodes() * 8) {
          // Recursively partition the coarse graph.
      ...
      // Partition the fine graph directly.
      partition_coarsest(fine_graph, fine_parts, num_parts);
