...
struct MetisPartition final {
  ...
  static void operator()(const Graph& graph, Parts&& parts, size_t num_parts) {
    ...
    xadj.reserve(graph.num_nodes() + 1), vwgt.reserve(graph.num_nodes());
    for (const auto& [node, weight] : graph.wnodes()) {
      xadj.push_back(static_cast<idx_t>(adjncy.size()));
      vwgt.push_back(static_cast<idx_t>(weight));
    ...
    if (const auto status = METIS_PartGraphKway(
            ...
            /* idx_t*  options = */ nullptr, // options.data(),
