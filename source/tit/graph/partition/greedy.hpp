...
///
/// On each iteration, we find the connected component with the smallest
/// node weight and associate parts within it in a BFS manner.
template<refine_func Refine = RefinePartsFM>
class GreedyPartition final {
public:

  /// Construct the greedy partitioning function.
  /// @{
  constexpr GreedyPartition() = default;
  constexpr explicit GreedyPartition(Refine refine) noexcept
