...
struct wnode_t final {
  ...
  template<tuple_like<node_t, weight_t> Arg>
    requires different_from<Arg, wnode_t>
  constexpr explicit(false) wnode_t(const Arg& arg) noexcept {
    *this = arg;
  }
...
struct wconn_t final {
  ...
  constexpr explicit(false) wconn_t(const Arg& arg) noexcept {
    *this = arg;
  }

  /// Assign a tuple-like argument to the weighted connection.
  template<class /*tuple_like<node_t, weight_t>*/ Arg>
    requires different_from<Arg, wconn_t>
...
struct edge_t final {
  ...
  template<tuple_like<node_t, node_t> Arg>
    requires different_from<Arg, edge_t>
  constexpr explicit(false) edge_t(const Arg& arg) noexcept {
    *this = arg;
  }
...
struct wedge_t final {
  ...
  template<tuple_like<node_t, node_t, weight_t> Arg>
    requires different_from<Arg, wedge_t>
  constexpr explicit(false) wedge_t(const Arg& arg) noexcept {
    *this = arg;
  }
...
template<class AdjacencyContainer = Multivector<node_t>>
...
public:
  ...
  constexpr auto transform_edges(Func func) const {
    return nodes() | std::views::transform([this, func](node_t node) {
             return this->edges(node) |
                    std::views::take_while([node](node_t neighbor) { //
                      return neighbor < node;
                    }) |
                    std::views::transform([node](node_t neighbor) {
                      /// @todo Use `edge_t` instead of `std::pair`.
                      return std::pair{node, neighbor};
                    }) |
...
public:
  ...
  /// Node weights.
  constexpr auto weights() const noexcept -> std::span<const weight_t> {
