...
struct wnode_t final {
  ...
  template<tuple_like2<node_t, weight_t> Arg>
...
struct edge_t final {
  ...
  template<tuple_like2<node_t, node_t> Arg>
...
struct wedge_t final {
  ...
  template<tuple_like2<node_t, node_t, weight_t> Arg>
...
public:
  ...
  constexpr auto nodes() const noexcept -> range_of<node_t> auto {
  ...
  /// Node weights.
  constexpr auto weights() const noexcept -> range_of<weight_t> auto {
    return std::views::repeat(unit_weight, num_nodes());
  }

  /// Range of the graph nodes and node weights.
  constexpr auto wnodes() const noexcept -> range_of<wnode_t> auto {
  ...
  constexpr auto edges(node_t node) const noexcept -> range_of<node_t> auto {
    TIT_ASSERT(node < num_nodes(), "Node index is out of range!");
    return (*this)[node];
  }

  /// Neighbors of the given node and the corresponding edge weights.
  constexpr auto wedges(size_t node) const noexcept -> range_of<wconn_t> auto {
    TIT_ASSERT(node < num_nodes(), "Node index is out of range!");
  ...
  /// @{
  constexpr auto edges() const noexcept -> range_of<edge_t> auto {
    return //
  ...
  constexpr auto wedges() const noexcept -> range_of<wedge_t> auto {
