...
public:
  ...
  void operator()(const Graph& graph, Parts& parts, size_t num_parts) const {
    ...
    const auto is_internal = [&graph, &parts](size_t node) {
      return std::ranges::all_of(graph.wedges(node),
                                 [&parts, node](const auto& conn) {
                                   return parts[conn.neighbor] == parts[node];
                                 });
    };
    ...
    const auto compute_gain = [&graph, &parts](node_t node, part_t to_part) {
      weight_t gain = 0;
    ...
    for (size_t iter = 0; iter < max_iter_; ++iter) {
      ...
      while (!gain_queue.empty()) {
        ...
        for (size_t test_part = 0; test_part < num_parts; ++test_part) {
          ...
          if (part_weights[test_part] + graph.weight(node) > max_part_weight) {
            continue;
          }
