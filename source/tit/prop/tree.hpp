...
public:
  ...
  auto to_bool() const -> std::optional<bool>;

  /// Try to convert the value to a 64-bit integer.
  auto to_int() const -> std::optional<int64_t>;

  /// Try to convert the value to a 64-bit floating-point number.
  auto to_real() const -> std::optional<float64_t>;
  ...
  /// Navigate to a leaf node at the dot-separated @p path.
  /// @{
  auto get_path(std::string_view path) -> Tree&;
  auto get_path(std::string_view path) const -> const Tree&;
  /// @}

  /// Return the keys of this map node.
  auto keys() const -> std::generator<std::string_view>;
...
/// Serialize a property tree to an indented JSON string.
auto tree_dump_json(const Tree& tree, int indent = 2) -> std::string;

// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

/// Build a property tree from command-line arguments.
auto tree_from_args(const std::vector<std::string_view>& args) -> Tree;

/// Build a property tree from a file (detected by extension).
auto tree_from_file(const std::filesystem::path& path) -> Tree;

/// Build a property tree from a JSON file.
auto tree_from_json(const std::filesystem::path& path) -> Tree;

/// Build a property tree from a YAML file.
