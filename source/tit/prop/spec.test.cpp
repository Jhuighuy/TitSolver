...
TEST_CASE("prop::BoolSpec::validate") {
  SUBCASE("success") {
    ...
    SUBCASE("string bool is converted") {
      const prop::BoolSpec spec{};
...
TEST_CASE("prop::IntSpec::validate") {
  SUBCASE("success") {
    ...
    SUBCASE("string integer is converted") {
      const auto spec = prop::IntSpec{}.range(1, 10);
      auto tree = prop::Tree{"5"};
      spec.validate(tree, "count");
      CHECK(tree.is_int());
...
TEST_CASE("prop::RealSpec::validate") {
  SUBCASE("success") {
    ...
    SUBCASE("string real is converted") {
      const prop::RealSpec spec{};
      auto tree = prop::Tree{"2.5"};
      spec.validate(tree, "x");
      CHECK(tree.is_real());
      CHECK(tree.as_real() == 2.5);
    }
  }
  SUBCASE("error") {
    SUBCASE("null without default") {
...
TEST_CASE("prop::StringSpec::validate") {
  SUBCASE("success") {
    ...
    SUBCASE("integer is converted to string") {
      const prop::StringSpec spec{};
      auto tree = prop::Tree{42};
      spec.validate(tree, "s");
      CHECK(tree.as_string() == "42");
    }
    SUBCASE("real is converted to string") {
      const prop::StringSpec spec{};
      auto tree = prop::Tree{3.14};
      spec.validate(tree, "s");
      CHECK(tree.as_string() == "3.14");
    }
    SUBCASE("boolean is converted to string") {
      const prop::StringSpec spec{};
      auto tree = prop::Tree{true};
      spec.validate(tree, "s");
      CHECK(tree.as_string() == "true");
    }
  }
  SUBCASE("error") {
    SUBCASE("null without default") {
...
TEST_CASE("prop::EnumSpec::validate") {
  SUBCASE("success") {
    ...
    SUBCASE("scalar is stringified before enum check") {
      const auto spec = prop::EnumSpec{}.option("true", "Enabled");
      auto tree = prop::Tree{true};
      spec.validate(tree, "mode");
      CHECK(tree.as_string() == "true");
    }
...
TEST_CASE("prop::AppSpec") {
  SUBCASE("success") {
    SUBCASE("type") {
      CHECK(prop::AppSpec{}.type() == prop::SpecType::App);
    }
    SUBCASE("name: set and retrieved") {
      const auto spec = prop::AppSpec{}.name("myapp");
      CHECK(spec.name() == "myapp");
    }
    SUBCASE("description: set and retrieved") {
      const auto spec = prop::AppSpec{}.description("Does things");
      CHECK(spec.description() == "Does things");
    }
  }
  SUBCASE("error") {
    SUBCASE("name: not set") {
      CHECK_THROWS_MSG(prop::AppSpec{}.name(),
                       tit::Exception,
                       "App name must be set");
    }
    SUBCASE("name: empty string") {
      CHECK_THROWS_MSG(prop::AppSpec{}.name(""),
                       tit::Exception,
                       "must not be empty");
    }
    SUBCASE("description: not set") {
      CHECK_THROWS_MSG(prop::AppSpec{}.description(),
                       tit::Exception,
                       "App description must be set");
    }
    SUBCASE("description: empty string") {
      CHECK_THROWS_MSG(prop::AppSpec{}.description(""),
                       tit::Exception,
                       "must not be empty");
    }
...
// Note: `spec_dump_help`, ``spec_dump_json`, `spec_dump_yaml_config` are
//       tested in `tit/prop/dump` test.
