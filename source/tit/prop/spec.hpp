...
/// Return whether the specification type is scalar.
auto spec_type_is_scalar(SpecType type) -> bool;

/// Return the canonical string representation of the specification type.
...
public:
  ...
  // Additional overloads for derived classes.
  template<class Self, std::derived_from<Spec> FieldSpec>
    requires (!std::same_as<std::remove_cvref_t<Self>, RecordSpec>)
  auto field(this Self&& self,
             std::string_view id,
             std::string_view name,
             FieldSpec spec) -> Self&& {
    static_cast<RecordSpec&&>(self).field(id, name, std::move(spec));
...
class AppSpec final : public RecordSpec {
public:

  /// Construct an application specification.
  AppSpec() = default;
  ...
  auto name(std::string name) && -> AppSpec&&;

  /// Get the application description.
  auto description() const -> std::string_view;

  /// Set the application description.
  auto description(std::string desc) && -> AppSpec&&;
...
/// Cast a specification to a concrete type (unsafely).
template<std::derived_from<Spec> T>
auto spec_cast(const Spec& spec) -> const T& {
  return static_cast<const T&>(spec); // NOLINT(*-static-cast-downcast)
}

// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

/// Presentation-oriented metadata derived from a specification.
struct SpecInfo final {
...
/// Derived metadata for a specification.
auto spec_info(const Spec& spec) -> SpecInfo;

// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

/// Print a JSON schema description of a record specification.
void spec_dump_json(const RecordSpec& spec, std::ostream& os);

/// Print a YAML config template for a record specification.
