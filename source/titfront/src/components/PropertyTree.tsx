...
import { PropertyEditor } from "~/components/PropertyEditors";
...
export function PropertyTree({ children }: PropertyTreeProps) {
  // ---- Folding. -------------------------------------------------------------

  type ID = string;

  const [unfoldedProperties, setUnfoldedProperties] = useState(new Set<ID>());
    ...
    <Flex direction="column">
      {flatProperties.map((property, propertyIndex) => (
        <Flex
          ...
          px="1"
          className={cn(
            propertyIndex % 2 === 0 && "bg-gray-300 dark:bg-gray-800",
...
export interface PropertySectionsSectionProps {
  name: string;
...
export function PropertySections({ children }: PropertySectionsProps) {
  ...
  return (
    <Flex direction="column" m="2" gap="4" className="hui">
      {/* ---- Sections. --------------------------------------------------- */}
