...
void bind_array_view(py::Module& m) {
  ...
  //      })
  c.def<"__hash__", [](const DataArray& self) {
    return static_cast<uint64_t>(self.id());
  }>();
  ...
  c.prop<"data", [](DataArray& self) { //
    auto data = self.open_read() | std::ranges::to<std::vector<byte_t>>();
...
void bind_dataset_view(py::Module& m) {
  ...
  //      })
  c.def<"__hash__",
        [](DataSet& self) { return static_cast<uint64_t>(self.id()); }>();
...
void bind_time_step_view(py::Module& m) {
  ...
  //        return &self.storage() == &other.storage() && self == other;
  //      })
  c.def<"__hash__", [](const TimeStep& self) {
    return static_cast<uint64_t>(self.id());
  }>();
...
void bind_series_view(py::Module& m) {
  ...
  //      })
  c.def<"__hash__",
        [](const Series& self) { return static_cast<uint64_t>(self.id()); }>();
