...
#include "tit/python/nb_utils.hpp"
...
// Make a Python object owned by the storage.
template<std::movable Object>
auto make_storage_owned(Object object, Storage& storage) -> nb::object {
  auto result = nb::cast(std::move(object));
  nb::detail::keep_alive(result.ptr(), nb::find(storage).ptr());
  return result;
}
template<std::movable Object>
auto make_storage_owned(Object object) -> nb::object {
  auto& storage = object.storage();
  return make_storage_owned(std::move(object), storage);
}
...
void bind_array_view(nb::module_& m) {
  ...
  nb::class_<DataArray>(m, "DataArray")
      ...
      .def_prop_ro("storage",
                   [](DataArray& self) { return nb::find(self.storage()); })
...
void bind_dataset_view(nb::module_& m) {
  ...
  nb::class_<DataSet>(m, "DataSet")
      ...
      .def_prop_ro("storage",
                   [](DataSet& self) { return nb::find(self.storage()); })
      .def_prop_ro("num_arrays", &DataSet::num_arrays)
      .def_prop_ro("arrays", &DataSet::arrays)
      .def("find_array", [](DataSet& self, const std::string& name) {
        const auto array = self.find_array(name);
        return array.has_value() ? make_storage_owned(*array) : nb::none();
      });
}
...
void bind_time_step_view(nb::module_& m) {
  ...
  nb::class_<TimeStep>(m, "TimeStep")
      ...
      .def_prop_ro("storage",
                   [](TimeStep& self) { return nb::find(self.storage()); })
      .def_prop_ro("time", &TimeStep::time)
      .def_prop_ro(
          "uniforms",
          [](TimeStep& self) { return make_storage_owned(self.uniforms()); })
      .def_prop_ro( //
          "varyings",
          [](TimeStep& self) { return make_storage_owned(self.varyings()); });
}
...
void bind_series_view(nb::module_& m) {
  ...
  nb::class_<Series>(m, "Series")
      ...
      .def_prop_ro("storage",
                   [](Series& self) { return nb::find(self.storage()); })
      ...
      .def_prop_ro("last_time_step", [](Series& self) {
        return make_storage_owned(self.last_time_step());
      });
}
...
void bind_data_storage(nb::module_& m) {
  ...
  nb::class_<Storage>(m, "Storage")
      ...
      .def("__repr__",
           [](const Storage& self) {
             return std::format("pytit.data.Storage('{}')",
                                self.path().string());
           })
      .def_prop_ro("path",
