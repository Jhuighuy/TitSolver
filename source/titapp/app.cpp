...
public:
  ...
  explicit WebSocketServer(QObject* parent = nullptr)
    ...
    if (server_->listen(QHostAddress::LocalHost,
      ...
      error("WS server is running on port {}.", server_->serverPort());
      connect(server_.get(),
  ...
  void onTextMessageReceived(const QString& message) {
    ...
    for (const auto* var : {"r", "rho"}) {
      ...
      response["result"][var] = std::span{
          safe_bit_ptr_cast<const float*>(std::as_const(r_data).data()),
          r_data.size() / sizeof(float),
      };
    }
...
// NOLINTBEGIN(*-use-internal-linkage,*-include-cleaner)
static void init_qt_resources() {
  Q_INIT_RESOURCE(resources);
}
// NOLINTEND(*-use-internal-linkage,*-include-cleaner)
...
void tit::main(CmdArgs args) {
  ...
  exit(static_cast<ExitCode>(QApplication::exec()));
}
