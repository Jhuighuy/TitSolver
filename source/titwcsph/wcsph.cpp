...
auto sph_main(CmdArgs /*args*/) -> int {
  ...
  info("Num. fluid particles: {}", num_fluid_particles);
...
struct DummyEquations {
  ...
  static constexpr auto modified_fields = required_fields - TypeSet{m, h};
};

auto sph_cl_main(CmdArgs /*args*/) -> int {
  ...
  for (size_t n = 0;; ++n) {
    ...
    if ((n % 100 == 0 && n != 0) || end) {
      ...
      queue.finish();
      particles.write(time * sqrt(g / H), series);
      info("Writing particles to file...");
    }
    if (end) break;
    time += dt;
  }
