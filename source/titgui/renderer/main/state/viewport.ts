 ...
 * Deliberately module-level: frame data is shared by all viewports — one
 * case, one storage, one timeline position.
 */
...
/**
 * Per-viewport state: one bundle per viewport tab. Frame data is shared
 * across viewports (one case, one timeline position); camera, field
 * selection, coloring, tool mode, and legend are per-viewport.
 */
export type ViewportState = ReturnType<typeof createViewportState>;

/**
 * Create a fresh viewport state bundle.
 */
// oxlint-disable-next-line max-lines-per-function -- an atom-graph definition.
export function createViewportState() {
  // ---- Tool and selection. ---------------------------------------------------

  const toolModeAtom = atom<ToolMode>("normal");

  /** Pending selection command; consumed by the renderer binding. */
  const selectionCommandAtom = atom<SelectionCommand | null>(null);

  const selectionCountAtom = atom(0);

  // ---- Camera. -----------------------------------------------------------------

  const projectionAtom = atom<Projection>("orthographic");
  const backgroundColorNameAtom = atom<BackgroundColorName>("default");
  const cameraPositionAtom = atom(new Vector3());
  const cameraRotationAtom = atom(new Euler());

  // ---- Field selection. ----------------------------------------------------

  /** Field selected by the user. May be absent from the current frame data. */
  const fieldNameAtom = atom("rho");
  ...
  const renderModeAtom = scopedAtom<RenderMode>(fieldScopeAtom, (get) =>
    get(fieldAtom).rank === 1 ? "glyphs" : "points",
  );

  const pointSizeAtom = atom(10);

  const glyphScaleModeAtom = scopedAtom<GlyphScaleMode>(
    fieldScopeAtom,
    () => "uniform",
  );
  ...
  const userColorFieldNameAtom = scopedAtom(fieldScopeAtom, (get) =>
    get(effectiveFieldNameAtom),
  );

  const colorFieldNameAtom = atom((get) => {
  ...
  const colorFieldModifierAtom = scopedAtom(coloringScopeAtom, (get) =>
    fieldModifierDefault(get(colorFieldAtom)),
  );

  const colorTitleAtom = atom((get) =>
  ...
  const colorMapNameAtom = scopedAtom<ColorMapName>(
    colorMappingScopeAtom,
    () => "jet",
  );

  const colorRangeModeAtom = scopedAtom<ColorRangeMode>(
    colorMappingScopeAtom,
    () => "auto",
  );
  ...
  const shadingModeAtom = atom<ShadingMode>("shaded");
  const legendEnabledAtom = atom(true);
  const legendTickCountAtom = atom(5);

  // ---- Renderer binding. -----------------------------------------------------

  /**
   * Bind a renderer to this viewport state: push the current state, keep the
   * renderer synchronized with atom changes, and pull camera movements back
   * into the atoms. Returns an unbind function.
   */
  function bindRenderer(renderer: Renderer) {
...
// Registry of viewport states by tab ID. States are kept for the window
// lifetime, so closing and reopening a viewport tab preserves its settings.
const viewportStates = new Map<string, ViewportState>();

/**
 * Get (or lazily create) the viewport state for the given tab ID.
 */
export function getViewportState(id: string) {
  let state = viewportStates.get(id);
  if (state === undefined) {
    state = createViewportState();
    viewportStates.set(id, state);
  }
  return state;
}
...
const ViewportStateContext = createContext<ViewportState | null>(null);

/**
 * Provider making a viewport state bundle available to the components of a
 * viewport tab.
 */
export const ViewportStateProvider = ViewportStateContext.Provider;

/**
 * The viewport state bundle of the enclosing viewport tab.
 */
export function useViewportState(): ViewportState {
  const state = useContext(ViewportStateContext);
  assert(state !== null, "Not inside a viewport tab.");
  return state;
}
