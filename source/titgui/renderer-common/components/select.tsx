...
function collectItems(node: ReactNode, out: Map<string, ReactNode>): void {
  Children.forEach(node, (child) => {
    ...
    if (child.type === SelectItem) {
      ...
      if (typeof val === "string" || typeof val === "number") {
        out.set(String(val), props.children as ReactNode);
      }
      return; // don't recurse into item children
    }
    if (props.children != null) {
      collectItems(props.children as ReactNode, out);
    }
  });
}
...
function SelectRoot<Value = string>({
  ...
  Omit<ComponentProps<typeof BaseSelect.Root>, "onValueChange"> & {
    value?: Value;
    defaultValue?: Value;
...
>) {
  ...
  const currentValue =
    valueProp === undefined ? internalValue : (valueProp as string);

  // Build item registry synchronously from the React element tree so it is
  // available on the very first render — before the popup is ever opened.
...
function SelectValue({
  ...
  ComponentProps<typeof BaseSelect.Value> & { placeholder?: string }
>) {
  return (
    <BaseSelect.Value
      {...props}
