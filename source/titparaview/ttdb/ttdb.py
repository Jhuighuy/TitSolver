...
"""A library for interacting with BlueTit storage files."""
...
import os
from typing import Any, Final, final, Callable, cast, NewType
import numpy as np
...
class __Library:
    __dll: Final[CDLL]
    __last_error: Final[Callable[[], bytes | None]]
    ...
    def __init__(self) -> None:
        ...
        for filename in ("libtit_ttdb.so", "libtit_ttdb.dylib", "tit_ttdb.dll"):
            ...
            raise RuntimeError("Unable to find the ttdb library.")
...
class ArrayIter(Iterator[Array]):
    ...
    __iter: Final[ttdb_array_iter_t]
...
class FrameIter(Iterator[Frame]):
    ...
    __iter: Final[ttdb_frame_iter_t]
...
class SeriesIter(Iterator[Series]):
    ...
    __iter: Final[ttdb_series_iter_t]
