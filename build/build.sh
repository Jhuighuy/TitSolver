...
COLUMNS=${COLUMNS:-$([ $TERM ] && tput cols || echo 80)}
...
usage() {
  ...
  echo "  -c, --config <config> Build configuration (Debug/Release/Coverage, default: Release)."
...
configure() {
  ...
  [ -n "$EXTRA_ARGS" ] && CMAKE_ARGS+=("$EXTRA_ARGS")
  "${CMAKE_ARGS[@]}" || exit $?
}
