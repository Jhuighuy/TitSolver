...
function(setup_python_requirements)
  # Install pip.
  execute_process(
    COMMAND "${CHRONIC_EXE}" "${Python3_EXECUTABLE}" -m ensurepip --upgrade
  ...
  execute_process(
    COMMAND
      "${CHRONIC_EXE}"
        "${Python3_EXECUTABLE}" -m pip install -r "${REQUIREMENTS_TXT}"
  ...
  foreach(FILE ${Python3_STDLIB_FILES})
    ...
    install(
      FILES
        "${Python3_STDLIB}/${FILE}"
      DESTINATION
        "${DESTINATION}/lib/python3.${Python3_VERSION_MINOR}/${FILE_DIR}"
      RENAME
        "${FILE_NAME}"
    )
  endforeach()
