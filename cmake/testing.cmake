...
function(add_tit_test)
  ...
  if(TEST_TARGET)
    ...
    list(POP_FRONT TEST_TARGET TEST_TARGET_NAME)
    list(PREPEND TEST_TARGET "$<TARGET_FILE:${TEST_TARGET_NAME}>")
    set(TEST_COMMAND ${TEST_TARGET})
  endif()

  # Should we run the test under MPI?
  if(TEST_MPI_RANKS)
    if(NOT TEST_MPI_RANKS MATCHES "^[1-9][0-9]*$")
      message(FATAL_ERROR "'MPI_RANKS' must be a positive integer.")
    endif()
    list(PREPEND TEST_COMMAND
      "${MPIEXEC_EXECUTABLE}"
      "${MPIEXEC_NUMPROC_FLAG}" "${TEST_MPI_RANKS}"
    )
  endif()
  ...
  # Reserve the processor slots for the MPI processes.
  if(TEST_MPI_RANKS)
    set_tests_properties("${TEST_NAME}"
